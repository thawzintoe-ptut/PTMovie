package com.ptut.pmovie.feature.utils
...
fun stringToLocalDateTime(dateAsString: String, df: DateTimeFormatter) =
    dateAsString.ignoreException { LocalDateTime.parse(dateAsString, df) }

fun String.ignoreException(r: RunnableException) {
    try {
        r.run()
    } catch (e: Exception) {
    }
...
fun interface RunnableException {
